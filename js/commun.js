...
function addPiezoToMap() {
		...
		*/
		...
		if (bnat && ((snbmes == '') || (rep[ipt][snbmes] >= nbanamin)) && ((datefin == '') || (rep[ipt][sdatefin] >= datefin)) && ((datedeb == '') || (rep[ipt][sdatedeb] <= datedeb))) {
			...
			if (typeof(scodalt) !== 'undefined') { markerPiezoFeature[ipt].set(scodalt, rep[ipt][scodalt]); }  // 2023-03-05
			...
			//console.table(markerPiezoFeature[ipt]);
...
function event_params() {
	...
	document.getElementById('val7').onchange = function() { 
		seuil_anc = seuil;
		if ((typeof(sres) !== 'undefined')  && (sres != '')) {  
			seuil = document.getElementById("val7").value;
			if (seuil_anc != seuil) { redrawStations();}
		}
	}
}